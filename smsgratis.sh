...
${RED}  
#############################################################################
[^]GUNAKAN DENGAN BIJAK JANGAN DISALAHGUNAKAN YA BRO..!! INGAT DOSA...!!![^]
                                   _______       .___      
...
/_____ \/____  >\___  >\___  >___  >\_____  /\____ |\___  >
      \/     \/     \/     \/    \/       \/      \/    \/ 
                   [^]SMS GRATISS BY ONE22[^]
#############################################################################
"

}
function negetgan(){
  getcookie=$(curl -s 'https://alpha.payuterus.biz/?a=keluar' -I \
           -H 'Connection: keep-alive' \
           -H 'Upgrade-Insecure-Requests: 1' \
           -H "User-Agent: $AGENT" \
           -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3' \
           -H 'Referer: https://alpha.payuterus.biz/send.php' --compressed|grep -Po "(?<=PHPSESSID=)[^,]*"|cut -d ';' -f1)
         bypass=$(curl -s 'https://alpha.payuterus.biz/?a=keluar' \
           -H 'Connection: keep-alive' \
           -H 'Upgrade-Insecure-Requests: 1' \
           -H "User-Agent: $AGENT" \
           -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3" \
           -H 'Referer: https://alpha.payuterus.biz/send.php' \
           -H "Cookie: PHPSESSID=${getcookie};" --compressed | grep "value" | grep "name" | grep -oP 'value="\K[^"]+')
          okeey1=$(curl -s 'https://alpha.payuterus.biz/?a=keluar' \
           -H 'Connection: keep-alive' \
           -H 'Upgrade-Insecure-Requests: 1' \
           -H "User-Agent: $AGENT" \
           -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3' \
         ...
         #  ok=`expr $okeey1 | bc`
         #fi
         okeey=$(curl -s "https://alpha.payuterus.biz/send.php" \
         -H 'Connection: keep-alive' \
         -H 'Pragma: no-cache' \
         ...
         if [[ $okeey =~ "SMS Gratis Telah Dikirim" ]]; then
             echo -e "${GREEN}[+] ${NORMAL}mantap berhasil kirim BRO"
         else
             echo -e "${RED}[+] ${NORMAL}gagal kirim nih BRO"
         fi
}
nebanner
echo -en "[+] NOMOR NYA :"; read now
echo -en "[+] ISI PESAN NYA Minimal 15 karakter :"; read psn
(
for x in okeey; do
